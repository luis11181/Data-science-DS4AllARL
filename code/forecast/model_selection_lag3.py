...
def preprocess_data(query, conn):
    ...
    #scaler = pickle.load(open('./forecast/scaler.pkl', 'rb'))
    scaler.fit(reals)
...
query = '''
SELECT * FROM model_query_lag3;
'''

scaler, X_train, y_train, X_test, y_test = preprocess_data(query, conn)
pickle.dump(scaler, open('./scaler_lag3.pkl', 'wb'))
...
pickle.dump(mod_res, open('./models_lag3.pkl', 'wb'))
